        ...
        if str == '/like.png':
            ...
            if last_s == 'LIKE.PNG' or last_s == 'G' or last_s == 'C':
                ...
                if self.STATES[self.state] == 'G':
                    self.last_generated_img_ID = re.search(r'_(img\d+_\w+)\.jpg', str).group(1)  # group 0: whole match, 1: () part
                ...
                logFile.write(url+'\n')
                if dest_path in self.saved_img_name:
                    logFile.write('CONFLICT: ' + url + ' ' + os.path.basename(dest_path) + '\n')
                    print('CONFLICT: ' + url + ' ' + os.path.basename(dest_path))
                    self.conflict_cnt += 1
                self.saved_img_name.add(dest_path)
...
logFile = open('allimgslink_NoOP.txt', 'w')
def main():

    # some initialization

    out_dir = './downloaded_NoOP'
    ...
    try:
        while True:
            ...
            print ('Finished extracting image url. Waiting for download threads to finish... Conflict Count: ' + str(downloader.conflict_cnt))
            ...
            logFile.write('NextURL: ' + nexturl + ' ' + time.asctime()+'\n')
            logFile.flush()
            url = nexturl
